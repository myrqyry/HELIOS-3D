  ...
  return (
    <div className="claims-table my-6">
      <div className="flex flex-wrap items-center gap-3 mb-4">
        <span className="font-sans text-xs text-parchment-2 uppercase tracking-wider font-bold mr-2">Filter Matrix:</span>
        {tags.map((tag) => (
          <label
            key={tag}
            className="inline-flex items-center gap-2 text-xs font-sans font-medium cursor-pointer bg-obsidian-2/50 hover:bg-obsidian-2 border border-obsidian-3/40 rounded px-2.5 py-1 text-parchment-2 hover:text-parchment focus-within:ring-2 focus-within:ring-amber focus-within:ring-offset-2 focus-within:ring-offset-obsidian transition-colors select-none"
          >
            <input
              type="checkbox"
        ...
        <button
          type="button"
          onClick={reset}
          className="text-xs text-parchment-2 hover:text-ember focus-visible:text-ember focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber focus-visible:ring-offset-2 focus-visible:ring-offset-obsidian ml-auto font-sans font-medium underline underline-offset-4 cursor-pointer"
        >
          Reset Filters
        </button>
      </div>
      ...
      <div className="overflow-x-auto rounded-xl border border-obsidian-3/30 bg-obsidian-2/30 backdrop-blur-sm shadow-xl">
        <table className="w-full text-sm border-collapse min-w-[600px]">
          <thead>
            <tr className="border-b border-obsidian-3/40 bg-obsidian-2/50 text-left text-amber font-sans text-xs uppercase tracking-wider">
          ...
          <tbody>
            {filtered.map((row, i) => (
              <tr key={i} className="border-b border-obsidian-3/30 hover:bg-obsidian-3/20 transition-colors duration-150">
                ...
                <td className="py-3.5 px-5 font-sans text-xs text-amber font-bold">{row.tag}</td>
