...
In early 2026, researchers at Tsinghua University (led by Dai Qionghai and Wu Jiamin) unveiled a completely revolutionary optical manipulation process known as the Digital Incoherent Synthesis of Holographic Light Fields (DISH). Published as a landmark study in Nature, DISH completely disrupted volumetric 3D printing by producing highly complex, millimeter-sized volumetric structures in an astonishing 0.6 seconds, achieving a volumetric printing rate of up to 333 cubic millimeters per second.

Unlike traditional, highly constrained stereolithography or high-resolution 3D printing methods that rely on time-consuming layer-by-layer or point-by-point laser scanning, DISH forms the entire structural volume simultaneously. It achieves this unprecedented speed by utilizing a complex multi-directional projection of coherent holographic light fields. Instead of mechanically rotating the sample container—which introduces severe fluid dynamics issues and physical alignment errors—a rapidly rotating periscope system handles all angle changes. The computational backend of the DISH system recalculates the complex holograms step-by-step for the different projection angles in real-time, requiring only an optically flat surface on the target container. This configuration results in the sub-second generation of highly complex, internally detailed 3D light distributions with verified physical resolutions down to 12 micrometers.
