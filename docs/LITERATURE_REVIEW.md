...
## 8. Programmable Chiral Organic Interface Layer (Speculative Side Branch)

These results are not part of the main HELIOS-3D compute path. They are tracked because they support a coherent long-range **chiral molecular sensing** branch that could later couple to a topological spintronic readout.

*   **Free-Standing 2D Peptide Crystals (anchor):** Wang et al., *Free-standing ultrathin two-dimensional peptide crystals* (Nature Chemistry 2026), demonstrate a metal-directed β-sheet-like assembly strategy that yields free-standing ultrathin 2D peptide crystals with programmable sequence, chirality, and side-chain chemistry. The crystals are exfoliated into single-crystalline nanosheets that stereoselectively bind glucocorticoids and chiral pharmaceutical molecules with **enantioselectivity up to 20.9**. `[DEMONSTRATED]`
*   **Significance for HELIOS:** The paper validates that programmable, chiral, atomically thin organic 2D crystals with side-chain-defined chemical recognition can be made and exfoliated. This is a candidate **molecular recognition layer** for a future HELIOS-style chiral spintronic sensing stack. `[PROPOSED]`
*   **Free-Standing 2D Woven Polymer Networks:** A 2024 Nature Chemistry study reports single crystals of purely organic free-standing 2D woven polymer networks with a monolayer thickness of about **1.3 nm**, establishing a broader materials class of free-standing organic 2D monolayers. `[DEMONSTRATED]`
*   **Free-Floating 2D Peptoid Sheets:** A 2010 Nature Materials study reported giant free-floating 2D crystalline sheets only **2.7 nm thick** from sequence-specific peptoid polymers, with direct TEM visualization and ligand-functionalized binding. This provides historical precedent for sequence-programmed organic nanosheets. `[DEMONSTRATED]`
*   **Speculative Stack (target framing only):** `target molecule → programmable chiral peptide crystal → spin-selective or proximity-coupled interface → vdW magnetic layer (skyrmion/hopfion host) → electrical, optical, or magneto-optical signal`.
*   **What this branch does NOT claim:** that peptide crystals are spintronic materials, generate skyrmions, solve magnetic readout, make 3D HELIOS fabrication easier, prove CISS coupling to the magnetic stack, are stable under laser switching or device heat, or can survive direct integration with $Fe_3GaTe_2$ processing. These are all open questions tracked in `OPEN_QUESTIONS.md`.
*   **Promotion gate:** promote from `[PROPOSED]` to a sensing candidate only after (1) transfer onto a relevant substrate without loss of crystallinity/function, (2) binding events couple to a measurable electrical, magnetic, or optical readout, and (3) the organic layer survives the relevant device conditions.
