...
### Q3.1: Super-Moiré Textures and Angle Precision
*   **Context:** Traditional "tear-and-stack" moiré engineering requires ultra-precise twist angles (e.g., 1.1°) that are difficult to maintain across a macroscale substrate.
*   **Update (June 2026):** The demonstration of **strain-induced moiré superlattices** using lithographically patterned stressor films (Cornell) provides a scalable path to mitigate this precision blocker. By "printing" the strain pattern, we can achieve sub-micron reproducibility and deterministic moiré geometries.
*   **Blocker:** How does this topological protection translate when the moiré potential is generated by strain in a non-planar 3D geometry? Can the stressor films be applied conformally to a DISH scaffold?
*   **Status:** `[PROPOSED]`
